<template>
  <div>
        ...
        <el-tabs v-model="data.dicOption" stretch @tab-click="handleTabClick">
          <el-tab-pane label="静态数据" name="static">
            ...
            <div style="margin-left: 22px">
              <el-button type="text" @click="handleAddFields"
                >添加列
              </el-button>
            </div>
          </el-tab-pane>
    ...
    <template v-if="data.type == 'select'">
      ...
      <el-form-item
        v-if="
          (data.cascaderItem || data.cascader) &&
          (data.cascaderItem || data.cascader).length > 0
        "
        label="级联默认选中"
        label-width="110px"
      >
...
export default {
  ...
  watch: {
    ...
    "data.sql": {
      handler(val) {
        this.data.dicQuery = { sql: val, dsName: this.data.dsName };
      },
      immediate: true,
    },
    dicFormatter: {
